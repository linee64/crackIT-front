...
export const AISimulator: React.FC = () => {
  ...
  const handleCheckSolution = () => {
    ...
    // Имитация задержки "размышления" ИИ
    setTimeout(() => {
      ...
      if (normalizedUser.includes(normalizedSolution.substring(0, 20)) || normalizedUser === normalizedSolution) {
        ...
        // Логика "умных" подсказок (упрощенная для примера)
        ...
        setFeedback({ type: 'hint', text: `🤖 Ментор: ${hint}` });
                  ...
                  {feedback.type !== 'none' ? (
                    ...
                    <div className="py-12 text-center space-y-4">
                      ...
                      <p className="text-slate-400 font-bold text-sm max-w-[200px] mx-auto leading-relaxed">
                        Реши задачу, чтобы получить менторский совет от AI.
                      </p>
                    </div>
                  )}
                </AnimatePresence>
